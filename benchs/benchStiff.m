function [Lndof,Tcpu]=benchStiff(d,Versions,LN,varargin)
%  benchMass : Benchmark for AssemblyStiffP1<version> functions using HyperCube(d,N) to generate meshes.
%
%  USAGE:
%    [Lndof,Tcpu]=benchStiff(d,Versions,LN)
%    [Lndof,Tcpu]=benchStiff(d,Versions,LN,'nbruns',3)
%    [Lndof,Tcpu]=benchStiff(d,Versions,LN,'isplot',true)
%
%  INPUTS:
%    d        : space dimension
...
%
%  SAMPLES:
%    [Lndof,Tcpu]=benchStiff(2,{'OptVS','OptV','OptV2','OptV1'},[50:50:250],5);
%    [Lndof,Tcpu]=benchStiff(3,{'OptVS','OptV','OptV2','OptV1'},[5:5:25],4);
%    [Lndof,Tcpu]=benchStiff(2,{'OptVS','OptV','OptV2'},[100:100:1000],'nbruns',1,'isplot',true);
%    [Lndof,Tcpu]=benchStiff(4,{'OptVS','OptV','OptV2'},[5:5:20],'nbruns',1,'isplot',true);
%    [Lndof,Tcpu]=benchStiff(5,{'OptVS','OptV','OptV2'},[2:2:8],'nbruns',1,'isplot',true);
%
  p = inputParser;
  if isOctave()
  ...
  for i=1:nLN
    ...
    for v=1:nV
      fprintf('    -> (%d/%d) Run AssemblyStiffP1%s (%dd)\n',v,nV,Versions{v},d);
      Assembly=eval(sprintf('@(Th) AssemblyStiffP1%s(Th);',Versions{v}));
      for l=1:nbruns
	start=tic();
	M=Assembly(Th);
	t(l)=toc(start);
	fprintf('         run (%d/%d) : %.4f(s)\n',l,nbruns,t(l))
      ...
      if v==1,M1=M;end
      Tcpu(i,v)=mean(t);
      fprintf('       AssemblyStiffP1%6s (%dd) : %d-by-%d matrix in %.4f(s)\n',Versions{v},d,size(M,1),size(M,2),Tcpu(i,v));
      if v>1, 
